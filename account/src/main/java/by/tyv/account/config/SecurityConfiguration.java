        ...
        return new TokenProvider() {
            ...
            public Mono<String> getCurrent() {
                return getContext().map(SecurityContext::getAuthentication)
                        ...
                        .map(token -> token.getToken().getClaims().get(USERNAME))
                        .map(String::valueOf);
            }
            ...
            @Override
            public Mono<String> getNewUserManagmentToken() {
                var req = OAuth2AuthorizeRequest
                        .withClientRegistrationId("user-manager-cc") // регистрация для клиента с правами manage-users
